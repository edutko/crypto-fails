...
func NewToken(username, realName string, roles []string) (string, error) {
	...
	return token.SignedString(config.JWTSigningKey())
}
...
func getKey(token *jwt.Token) (interface{}, error) {
	...
	if jku, ok := token.Header["jku"]; ok {
		if jwksURL, ok := jku.(string); ok {
			...
			if err == nil {
				...
				if err == nil {
					return publicOrBytes(jwKey), nil
				}
			}
		}
	}
	...
	if keyBytes, ok := getLocalKey(keyId); ok {
...
func getLocalKey(keyId string) ([]byte, bool) {
	keys := config.JWTSigningKeys()
	k, ok := keys[keyId]
	return k.PublicPEM, ok
}

func publicOrBytes(jwk jwkset.JWK) any {
	type publicKeyer interface {
		Public() crypto.PublicKey
	}
	key := jwk.Key()
	if pk, ok := key.(publicKeyer); ok {
		return pk.Public()
	}
	return key
}
