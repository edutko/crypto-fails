...
func TestNewToken(t *testing.T) {
	config.InitializeForTesting(config.Config{
		JWTSigningKey: bytes.Repeat([]byte{0x00}, 32),
	})
...
func TestParseToken(t *testing.T) {
	...
	config.InitializeForTesting(config.Config{
		JWTSigningKey: bytes.Repeat([]byte{0x00}, 32),
		JWTSigningKeys: map[string]crypto.KeyPair{
			"1": {PublicPEM: k1},
			"2": {PublicPEM: pem.EncodeToMemory(&pem.Block{
				Type:  "PUBLIC KEY",
				Bytes: ecPubBytes,
			})},
		},
...
func expiredToken(t *testing.T, username string) string {
	...
	s, err := token.SignedString(config.JWTSigningKey())
	assert.NoError(t, err)
	return s
}
