...
func (s *FSStore) ListObjects() ([]Metadata, error) {
	return s.ListObjectsWithPrefix("")
}

func (s *FSStore) ListObjectsWithPrefix(prefix string) ([]Metadata, error) {
...
func (s *FSStore) recursiveList(dir realPath, prefix string) ([]Metadata, error) {
	root := string(dir)
	if root == "" || root == ".." {
		root = "."
	}
