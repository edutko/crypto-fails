...
func TestInsecureBytes(t *testing.T) {
	...
	// note: this assumes the RNG is starting from the beginning, i.e. at an
	// even multiple of len(expected)
	b = InsecureBytes(64)
	assert.Len(t, b, 64)
	assert.Equal(t, bytes.Repeat(expected, 64/len(expected)), b)
}

func TestInsecureHex(t *testing.T) {
	SetWeakPRNG(weakprng.MT19937)
	...
	assert.Len(t, s, 1)
	assert.Regexp(t, regexp.MustCompile(`^[0-9a-f]$`), s)
	...
	assert.Len(t, s, 255)
	assert.Regexp(t, regexp.MustCompile(`^[0-9a-f]+$`), s)
	assert.NotEqual(t, strings.Repeat("0", 255), s)
}
