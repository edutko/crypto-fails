...
func TestGCMEncrypter_randomNonce(t *testing.T) {
	...
	assert.Len(t, w.Bytes(), len(plaintext)+len(nonce)+16)
	assert.Equal(t, plaintext, gcmUnseal(w.Bytes()[:12], w.Bytes()[12:]))
}
...
func TestGCMEncrypter_providedNonce(t *testing.T) {
	...
	assert.Len(t, w.Bytes(), len(plaintext)+16)
