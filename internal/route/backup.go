...
func startBackup(uid string) (string, error) {
	...
	files, err := stores.FileStore().ListObjects(uid)
	if err != nil {
		return "", err
	}
...
func createZip(uid string, j job.Descriptor, files []blob.Metadata) {
	...
	for _, m := range files {
		err = zipFile(zw, m.Key, strings.TrimPrefix(m.Key, uid+"/"))
		if err != nil {
			log.Println(err)
			continue
		}
